...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($distance <= 100) {
        ...
        include "conn.php";
        $stmt = mysqli_prepare($conn, "INSERT INTO attendance (time_in) VALUES (?)");
        ...
        // Insert the attendance record into the database with the in_office "remote"
include "conn.php";
$stmt = mysqli_prepare($conn, "INSERT INTO attendance (time_in, in_office) VALUES (?, ?)");
$in_office = 0;
mysqli_stmt_bind_param($stmt, 'si', $time, $in_office);
mysqli_stmt_execute($stmt);
mysqli_stmt_close($stmt);
mysqli_close($conn);
